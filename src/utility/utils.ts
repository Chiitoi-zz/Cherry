...
export const validate = async (client: AkairoClient) => {
    const { botChannelIds, categoryIds, checkChannelId, ignoreIds, interval, logChannelId, prefix, presenceStatus, serverIds, status } = client.config
    ...
    await delay(7000 * serverIds.length)
    ...
    const settings: Settings = {
        ...
        'PRESENCE_STATUS': { items: presenceStatus, invalid: presenceStatus && !['dnd', 'idle', 'invisible', 'online'].includes(process.env.PRESENCE_STATUS || '') },
        'SERVER_IDS': { items: serverIds, invalidItems: [...new Set(serverIds)].filter(serverId => !guildCache.has(serverId))   },
    ...
    for (const [setting, { items, invalid, invalidItems }] of Object.entries(settings)) {
        const colour = ['CATEGORY_IDS', 'CHECK_CHANNEL_ID', 'SERVER_IDS'].includes(setting) ? 'red' : 'yellow'
        ...
        if ((setting === 'SERVER_IDS') && invalidItems.length) {
            const message = !guildCache?.size ? `${ botName } is not in any server.` : `${ botName } is not in the servers with the following IDs: ${ formatChannels(invalidItems, 'server') }.`
